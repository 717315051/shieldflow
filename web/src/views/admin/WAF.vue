...
const overview = reactive({
  today_blocked: 0, total_blocked: 0, active_rules: 0, blocked_ips: 0,
})
...
const pieOption = computed(() => ({
  ...
  legend: { bottom: 0 },
  series: [{
    name: '威胁类型', type: 'pie', radius: ['40%', '70%'],
...
async function loadDashboard() {
  ...
  try {
    ...
    Object.assign(overview, {
      today_blocked: d.today_blocked ?? 0, total_blocked: d.total_blocked ?? 0,
      active_rules: d.active_rules ?? 0, blocked_ips: d.blocked_ips ?? 0,
    })
...
const configForm = reactive({
  enabled: false, mode: 'block', detection_level: 'medium',
  sql_injection: true, xss: true, path_traversal: true, command_injection: true,
  cc_protection: true, scanner_block: true, rate_limit: 100,
  block_duration: 3600, log_retention: 30, whitelist: '',
})
...
const logQuery = reactive({
  domain: '', ip: '', threat_type: undefined, start_time: undefined, end_time: undefined,
  page: 1, page_size: 10,
})
...
const logColumns = [
  ...
  { title: '客户端IP', dataIndex: 'ip', width: 140 },
  { title: '威胁类型', dataIndex: 'threat_type', width: 120 },
  { title: '规则', dataIndex: 'rule' },
  { title: '匹配内容', dataIndex: 'match', ellipsis: true },
  { title: '动作', dataIndex: 'action', width: 100, key: 'action' },
...
function resetLogQuery() {
  Object.assign(logQuery, {
    domain: '', ip: '', threat_type: undefined, start_time: undefined, end_time: undefined, page: 1,
  })
  loadLogs()
}
...
const trendOption = computed(() => ({
  ...
  series: trendSeries.value.map((s) => ({ name: s.name, type: 'line', smooth: true, data: s.data })),
}))
...
<template>
  <div class="page-container">
    <SfPageHeader title="WAF 管理" sub="Web 应用防火墙仪表盘、配置与日志" :show-refresh="true" @refresh="reloadCurrent" />
    ...
    <a-tabs v-model:activeKey="activeTab" @change="onTabChange">
      <a-tab-pane key="dashboard" tab="仪表盘">
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <SfTableCard title="威胁类型分布" :show-search="false" :show-refresh="false">
          ...
          <a-col :span="12">
            <SfTableCard title="Top 10 攻击 IP" :show-search="false" :show-refresh="false">
              <a-table
      ...
      <a-tab-pane key="config" tab="配置">
        <SfTableCard title="WAF 配置" sub="拦截模式、检测级别与防护规则" :show-search="false" :show-refresh="false">
      ...
      <a-tab-pane key="logs" tab="拦截日志">
        <SfPageHeader title="拦截日志" sub="WAF 拦截事件明细" :show-refresh="true" @refresh="loadLogs" />

        <SfTableCard title="日志列表" :show-search="false" :show-refresh="false">
          <template #filters>
            <a-input v-model:value="logQuery.domain" placeholder="域名" allow-clear style="width: 160px" />
            <a-input v-model:value="logQuery.ip" placeholder="客户端IP" allow-clear style="width: 140px" />
            <a-select v-model:value="logQuery.threat_type" placeholder="威胁类型" allow-clear style="width: 150px">
            ...
            <a-button type="primary" @click="logQuery.page = 1; loadLogs()">
          ...
          <a-table
            ...
            :loading="loading"
            row-key="id"
            :pagination="{ current: logQuery.page, pageSize: logQuery.page_size, total: logTotal, showSizeChanger: true, showTotal: (t) => `共 ${t} 条` }"
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'action'">
                <SfStatusBadge
                  :status="record.action === 'block' ? 'danger' : (record.action === 'captcha' ? 'warning' : 'info')"
                  :text="record.action === 'block' ? '阻断' : (record.action === 'captcha' ? '人机验证' : '观察')"
                />
              </template>
              <template v-else-if="column.key === 'status'">
                <SfStatusBadge
                  :status="record.status === 'success' ? 'success' : 'neutral'"
      ...
      <a-tab-pane key="analysis" tab="攻击分析">
        <SfPageHeader title="攻击分析" sub="按时间窗口聚合的威胁拦截趋势" :show-refresh="true" @refresh="loadAnalysis">
          <template #extra>
        ...
        <SfTableCard title="威胁类型趋势" :show-search="false" :show-refresh="false">
          <div style="padding: 16px">
