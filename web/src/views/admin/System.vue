...
const saveGeneral = async () => { try { await adminSystemApi.updateSettings(generalForm.value); message.success('保存成功') } catch(e) { message.error('保存失败') } }
...
<template>
  <div class="page-container">
    <SfPageHeader title="系统设置" sub="全局参数、DNS、ACME、告警与 AI 配置" />

    <a-tabs v-model:activeKey="activeTab">
      <a-tab-pane key="general" tab="全局设置">
        <SfTableCard title="全局设置" sub="站点信息、注册策略" :show-search="false" :show-refresh="false">
          <a-form :model="generalForm" layout="vertical" style="max-width: 720px">
            ...
            <a-form-item>
              <a-button type="primary" @click="saveGeneral">
      ...
      <a-tab-pane key="dns" tab="DNS设置">
        <SfTableCard title="DNS 设置" sub="默认 DNS 服务商与同步策略" :show-search="false" :show-refresh="false">
          <a-form :model="dnsForm" layout="vertical" style="max-width: 720px">
      ...
      <a-tab-pane key="acme" tab="ACME设置">
        <SfTableCard title="ACME 设置" sub="证书签发机构与验证方式" :show-search="false" :show-refresh="false">
          <a-form :model="acmeForm" layout="vertical" style="max-width: 720px">
            ...
            <a-form-item>
              <a-button type="primary" @click="saveACME">
                <template #icon><SfIcon name="SaveOutlined" :size="14" /></template>
                保存
              </a-button>
            </a-form-item>
      ...
      <a-tab-pane key="grpc" tab="gRPC配置">
        <SfTableCard title="gRPC 配置" sub="日志上传模式" :show-search="false" :show-refresh="false">
          <a-form :model="grpcForm" layout="vertical" style="max-width: 720px">
            ...
            <a-form-item>
              <a-button type="primary" @click="saveGRPC">
                <template #icon><SfIcon name="SaveOutlined" :size="14" /></template>
                保存
              </a-button>
      ...
      <a-tab-pane key="alert" tab="告警设置">
        <SfTableCard title="告警设置" sub="Telegram/邮件告警通道" :show-search="false" :show-refresh="false">
          <a-form :model="alertForm" layout="vertical" style="max-width: 720px">
            ...
            <a-form-item>
              <a-button type="primary" @click="saveAlert">
                <template #icon><SfIcon name="SaveOutlined" :size="14" /></template>
                保存
              </a-button>
            </a-form-item>
      ...
      <a-tab-pane key="ai" tab="AI配置">
        <SfTableCard title="AI 配置" sub="AI 服务商、模型与防护开关" :show-search="false" :show-refresh="false">
          <a-form :model="aiForm" layout="vertical" style="max-width: 720px">
            ...
            <a-form-item>
              <a-button type="primary" @click="saveAI">
                <template #icon><SfIcon name="SaveOutlined" :size="14" /></template>
                保存
              </a-button>
            </a-form-item>
      ...
      <a-tab-pane key="version" tab="版本信息">
        <SfTableCard title="版本信息" sub="构建版本、依赖与已安装组件" :show-search="false" :show-refresh="false">
          <a-descriptions :column="2" bordered>
            ...
            <a-descriptions-item label="已安装组件" :span="2">
              <a-space wrap>
                <SfStatusBadge status="success" text="后端API" />
                <SfStatusBadge status="success" text="gRPC服务" />
                <SfStatusBadge status="info" text="边缘节点" />
                <SfStatusBadge status="info" text="DNS同步" />
                <SfStatusBadge status="warning" text="日志服务器" />
              </a-space>
            </a-descriptions-item>
          </a-descriptions>
          <a-button type="primary" style="margin-top: 16px" @click="checkUpgrade">
            <template #icon><SfIcon name="CloudDownloadOutlined" :size="14" /></template>
            检查更新
          </a-button>
        </SfTableCard>
      </a-tab-pane>
